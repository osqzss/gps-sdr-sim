...
// with external 60dB attenuation
...
int main(int argc, char *argv[])
{
	...
	for (iumd=0; iumd<(numd-1); iumd++)
	{
		...
		for (isamp=0; isamp<2*IQ_BUFF_SIZE; isamp++)
		{
			...
			for (i=0; i<nsat; i++)
#ifdef _HACKRF
				iq_buff[isamp] += (signed char)(chan[i].iq_buff[isamp]>>4); // 12-bit bladeRF -> 8-bit HackRF
#else
